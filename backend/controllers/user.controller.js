...
export const sendSignupOtp = async (req, res) => {
     try {
          ...
          if (user && user.accountStatus !== 'Unverified') {
               return res.status(400).json({ message: 'User already exists and is verified.', success: false });
          }
          ...
          if (user && user.accountStatus === 'Unverified') {
               ...
               user = await User.create({
                    fullName, username, email, phone, password: hashedPassword, role,
                    profilePhoto: profilePhoto || '/assets/images/default-avatar.jpg',
                    otp, otpExpires, accountStatus: 'Unverified'
               });
          }
...
export const verifySignupOtp = async (req, res) => {
     try {
          ...
          return res.status(200).json({ message, success: true, status: user.accountStatus });
     } catch (error) {
...
export const sendLoginOtp = async (req, res) => {
     try {
          ...
          if (!isPasswordMatch) return res.status(400).json({ message: "Incorrect password.", success: false });

          if (user.role !== role) return res.status(400).json({ message: "Account doesn't exist with current role.", success: false });

          if (user.accountStatus === 'Unverified') {
               return res.status(400).json({ message: "Account unverified. Please sign up again.", success: false });
          }
          if (user.accountStatus === 'Pending') {
               return res.status(403).json({ message: "Your recruiter account is pending admin approval.", success: false });
          }
          ...
          if (!emailSent) return res.status(500).json({ message: "Failed to send OTP email.", success: false });
...
export const verifyLoginOtp = async (req, res) => {
     try {
          ...
          const userToSend = {
               ...
               profilePhoto: user.profilePhoto, bio: user.bio, skills: user.skills,
               resume: user.resume, videoPitch: user.videoPitch,
               isPortfolioPublic: user.isPortfolioPublic, portfolioTheme: user.portfolioTheme
          };

          return res.status(200)
               .cookie("token", token, { maxAge: 1 * 24 * 60 * 60 * 1000, httpOnly: true, sameSite: 'strict' })
               .json({ message: `Welcome back ${user.fullName}`, user: userToSend, token, success: true });
     } catch (error) {
...
export const getPublicProfile = async (req, res) => {
     try {
          ...
          if (!user) return res.status(404).json({ message: "Portfolio not found or is set to private.", success: false });
...
export const updateProfile = async (req, res) => {
     try {
          const userId = req.id;
          if (!userId) return res.status(401).json({ success: false, message: "Unauthorized: User ID not found." });

          const user = await User.findById(userId);
          if (!user) return res.status(404).json({ message: "User not found.", success: false });

          const { fullName, phone, bio, skills, password, isPortfolioPublic, portfolioTheme } = req.body;
          ...
          if (skills !== undefined) {
               ...
               else if (typeof skills === 'string') user.skills = skills.split(',').map(s => s.trim()).filter(s => s !== '');
               else user.skills = [];
          }
          ...
          if (req.files && req.files['profilePhoto'] && req.files['profilePhoto'][0]) {
               ...
               if (user.profilePhoto && user.profilePhoto.includes('res.cloudinary.com')) {
                    ...
                    if (publicIdMatch && publicIdMatch[1]) await cloudinary.uploader.destroy(`profile_photos/${publicIdMatch[1]}`).catch(() => { });
               }
               const fileUri = getDataUri(profilePhotoFile);
               const cloudResponse = await cloudinary.uploader.upload(fileUri.content, { folder: 'profile_photos', resource_type: 'image' });
          ...
          if (user.role === 'student' && req.files && req.files['resume'] && req.files['resume'][0]) {
               ...
               if (user.resume && user.resume.includes('res.cloudinary.com')) {
                    ...
                    if (publicIdMatch && publicIdMatch[1]) await cloudinary.uploader.destroy(`resumes/${publicIdMatch[1]}`, { resource_type: 'raw' }).catch(() => { });
               }
               const fileUri = getDataUri(resumeFile);
               const cloudResponse = await cloudinary.uploader.upload(fileUri.content, { folder: 'resumes', resource_type: 'raw' });
          ...
          if (user.role === 'student' && req.files && req.files['videoPitch'] && req.files['videoPitch'][0]) {
               ...
               if (user.videoPitch && user.videoPitch.includes('res.cloudinary.com')) {
                    ...
                    if (publicIdMatch && publicIdMatch[1]) await cloudinary.uploader.destroy(`video_pitches/${publicIdMatch[1]}`, { resource_type: 'video' }).catch(() => { });
               }
               const fileUri = getDataUri(videoPitchFile);
               const cloudResponse = await cloudinary.uploader.upload(fileUri.content, { folder: 'video_pitches', resource_type: 'video' });
...
export const getAppliedJobs = async (req, res) => {
     try {
          ...
          if (!user || user.role !== 'student') return res.status(403).json({ message: "Access denied.", success: false });
...
export const getAllApplicants = async (req, res) => {
     try {
          ...
          const applicants = await User.find({ 'appliedJobs.job': { $in: postedJobIds } })
               .select('fullName email phone resume videoPitch appliedJobs profilePhoto')
               .populate({
                    path: 'appliedJobs.job', match: { _id: { $in: postedJobIds } },
                    select: 'title company location jobType',
                    populate: { path: 'company', select: 'name' }
               });
               ...
               applicant.appliedJobs.forEach(app => {
                    if (postedJobIds.some(jobId => jobId.equals(app.job._id))) {
                         allApplicantsData.push({
                              _id: app._id, job: app.job, appliedDate: app.appliedDate, status: app.status,
                              user: { _id: applicant._id, fullName: applicant.fullName, email: applicant.email, phone: applicant.phone, resume: applicant.resume, videoPitch: applicant.videoPitch, profilePhoto: applicant.profilePhoto }
                         });
                    }
               });
          });
...
export const updateApplicationStatus = async (req, res) => {
     try {
          ...
          if (!['Pending', 'Selected', 'Rejected'].includes(status)) return res.status(400).json({ message: "Invalid status provided.", success: false });

          const user = await User.findOne({ 'appliedJobs._id': id });
          if (!user) return res.status(404).json({ message: "Application not found.", success: false });

          const application = user.appliedJobs.id(id);
          const job = await Job.findById(application.job);
          if (!job || job.created_by.toString() !== req.id.toString()) return res.status(403).json({ message: "Unauthorized.", success: false });

          application.status = status;
          await user.save();

          res.status(200).json({ message: "Status updated successfully.", success: true, application });
     } catch (error) {
          res.status(500).json({ message: "Internal Server Error", success: false, error: error.message });
     }
};
...
export const applyJob = async (req, res) => {
     try {
          ...
          if (!user || user.role !== 'student') return res.status(403).json({ message: "Only students can apply.", success: false });
          ...
          user.appliedJobs.push({ job: jobId, appliedDate: new Date(), status: 'Pending' });
          await user.save();
